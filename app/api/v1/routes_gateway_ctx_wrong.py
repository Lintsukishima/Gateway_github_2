...
ZERO_HIT_RETRY_ENABLED = os.getenv("GATEWAY_CTX_ZERO_HIT_RETRY", "1").strip().lower() in ("1", "true", "yes")
GARBLED_KW_REPAIR_ENABLED = os.getenv("GATEWAY_CTX_GARBLED_KW_REPAIR", "1").strip().lower() in ("1", "true", "yes")
...
def _fallback_kw_by_text(text: str) -> str:
    """Fallback keywords aligned with legacy behavior.

    - emotional/smalltalk-ish text -> 猫咪,哥哥
    - otherwise -> 撒娇,哥哥
    """
    return "猫咪,哥哥" if _is_emo_chitchat(text) else "撒娇,哥哥"


def _looks_garbled_keyword(keyword: str) -> bool:
    """Detect obvious mojibake / lost-char keyword such as '??,???' or replacement chars.

    We intentionally keep this conservative: only repair when high confidence.
    """
    kw = (keyword or "").strip()
    if not kw:
        return True
    ...
    # If the keyword is mostly '?' tokens, it's likely already corrupted by client encoding.
    ...
    if not tokens:
        return True
    bad = sum(1 for t in tokens if set(t) <= {"?"})
    return bad >= max(1, len(tokens))


def _is_zero_hit_response(outs: Dict[str, str], ctx: str) -> bool:
    text = "\n".join([ctx or "", *(str(v or "") for v in (outs or {}).values())]).lower()
    return (
        "hits: 0" in text
        or "kb hits: 0" in text
        or "no hit" in text
...
def _loads_json_lenient(raw: bytes) -> Any:
    """Parse JSON with tolerant charset handling for Windows/ngrok clients.

    Some clients (notably old PowerShell setups) may send UTF-16/GBK payloads
    while declaring plain application/json. We try common decoders in order.
    """
    if not raw:
        return {}

    encodings = ("utf-8", "utf-8-sig", "utf-16", "utf-16le", "utf-16be", "gb18030", "gbk", "cp936")
    ...
    # last resort: attempt with replacement, then fail to parse upstream caller style
...
async def _handle_jsonrpc(request: Request, msg: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if not keyword:
        keyword = _fallback_kw_by_text(text)

    if GARBLED_KW_REPAIR_ENABLED and _looks_garbled_keyword(keyword):
        old_keyword = keyword
        keyword = _fallback_kw_by_text(text)
        print(f"[gateway_ctx] repair_garbled_kw from={old_keyword!r} to={keyword!r}")
